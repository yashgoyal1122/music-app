...
const DisplayAblum = () => {
  ...
  return (
    ...
    <div className=' mt-10  flex gap-8 flex-col md:flex-row md:items-end '>
      <img className='w-48 rounded-2xl ' src={albumData.image} alt="" />
      <div className='flex flex-col'>
        <p>Playlist</p>
        <h2 className='text-5xl font-bold mb-4 md:text-7xl '>{albumData.name}</h2>
        <h4>{albumData.desc}</h4> 
        <div className=' mt-1 flex items-center gap-1 justify-between'>
          <img className='inline-block w-10' src={assets.my_logo} alt="" />
          <div className=' mt-1 flex items-center gap-5 justify-between'>
          <b className='font-semibold '>My Music</b>
          •12,34,567 Likes ❤️
